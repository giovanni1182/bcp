...
public class EstatisticaMorosidadeView extends PortalView
{
	private Aseguradora aseguradora;
	private Date dataFim;
	private boolean listar;
	
	public EstatisticaMorosidadeView(Aseguradora aseguradora, Date dataFim, boolean listar) throws Exception
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		Button consultarButton = new Button("Consultar en la Pantalha",new Action("estatisticaMorosidade"));
		block.add(new Space(25));
		block.add(consultarButton);
		block.add(new Space(4));
		consultarButton = new Button("Generar Excel",new Action("estatisticaMorosidade"));
		...
		if(listar)
		{
			...
			for(Iterator<Aseguradora> i = aseguradoras.iterator() ; i.hasNext() ; )
			{
				...
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String linhaSuja = home.obterDadosAquivoMorosidade(aseg, c.getTime());
					...
					double prima2 = Double.parseDouble(linha[1]);
					valorMap.put(mesAno, prima2);
					
					c.add(Calendar.MONTH, 1);
				}
				
				c.setTime(dataInicio);
				table2.add("Valor de la Deuda en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
					double valor = 0;
