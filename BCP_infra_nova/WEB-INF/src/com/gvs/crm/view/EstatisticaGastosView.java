...
public class EstatisticaGastosView extends PortalView
{
	...
	public EstatisticaGastosView(Aseguradora aseguradora, Date dataFim, boolean listar) throws Exception
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		Button consultarButton = new Button("Consultar en la Pantalha",new Action("estatisticaGastos"));
		block.add(new Space(25));
		block.add(consultarButton);
		block.add(new Space(4));
		consultarButton = new Button("Generar Excel",new Action("estatisticaGastos"));
		...
		if(listar)
		{
			...
			for(Iterator<Aseguradora> i = aseguradoras.iterator() ; i.hasNext() ; )
			{
				...
				c.setTime(dataInicio);
				table2.add("Importe pago al Liquidador en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
				...
				c.setTime(dataInicio);
				table2.add("Importe pago al Asegurado en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
				...
				c.setTime(dataInicio);
				table2.add("Importe pago al Tercero en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
				...
				c.setTime(dataInicio);
				table2.add("Importe pago a Otros en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
	...
	{
		return new Label("Estadística Gastos");
	}
}