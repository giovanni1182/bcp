...
public class EstatisticaReasegurosView extends PortalView
{
	private Aseguradora aseguradora;
	private Date dataFim;
	private boolean listar;
	
	public EstatisticaReasegurosView(Aseguradora aseguradora, Date dataFim, boolean listar) throws Exception
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		Button consultarButton = new Button("Consultar en la Pantalha",new Action("estatisticaReaseguros"));
		block.add(new Space(25));
		block.add(consultarButton);
		block.add(new Space(4));
		consultarButton = new Button("Generar Excel",new Action("estatisticaReaseguros"));
		...
		if(listar)
		{
			...
			for(Iterator<Aseguradora> i = aseguradoras.iterator() ; i.hasNext() ; )
			{
				...
				c.setTime(dataInicio);
				table2.add("Capital Reasegurado en Gs");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
					double valor = 0;
	...
	{
		return new Label("Estadística Reaseguros");
	}
}