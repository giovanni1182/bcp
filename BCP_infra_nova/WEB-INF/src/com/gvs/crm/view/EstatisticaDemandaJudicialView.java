...
public class EstatisticaDemandaJudicialView extends PortalView
{
	private Aseguradora aseguradora;
	private Date dataFim;
	private boolean listar;
	
	public EstatisticaDemandaJudicialView(Aseguradora aseguradora, Date dataFim, boolean listar) throws Exception
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		Button consultarButton = new Button("Consultar en la Pantalha",new Action("estatisticaDemandaJudicial"));
		block.add(new Space(25));
		block.add(consultarButton);
		block.add(new Space(4));
		consultarButton = new Button("Generar Excel",new Action("estatisticaDemandaJudicialExcel"));
		block.add(consultarButton);
		...
		Map<String,Double> montanteMap = new TreeMap<String,Double>();
		...
		if(listar)
		{
			...
			for(Iterator<Aseguradora> i = aseguradoras.iterator() ; i.hasNext() ; )
			{
				...
				while(c.getTime().compareTo(dataFim)<=0)
				{
					...
					String linhaSuja = home.obterQtdeMontanteDemandaJudicial(aseg, c.getTime());
					
					String[] linha = linhaSuja.split(";");
					String qtde = linha[0];
					double montante = Double.parseDouble(linha[1]);
					montanteMap.put(mesAno, montante);
				...
				c.setTime(dataInicio);
				table2.add("Monto demandado");
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String mesAno = (new SimpleDateFormat("MM/yyyy").format(c.getTime()));
					
					double montante = 0;
					if(montanteMap.containsKey(mesAno))
						montante = montanteMap.get(mesAno);
	...
	{
		return new Label("Estadística Demanda Judicial");
	}
}