...
public class EstatisticaAseguradoView extends PortalView
{
	private Aseguradora aseguradora;
	private Date dataFim;
	private boolean listar;
	
	public EstatisticaAseguradoView(Aseguradora aseguradora, Date dataFim, boolean listar) throws Exception
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		Button consultarButton = new Button("Consultar en la Pantalha",new Action("estatisticaAsegurado"));
		block.add(new Space(25));
		block.add(consultarButton);
		block.add(new Space(4));
		consultarButton = new Button("Generar Excel",new Action("estatisticaAsegurado"));
		...
		if(listar)
		{
			...
			for(Iterator<Aseguradora> i = aseguradoras.iterator() ; i.hasNext() ; )
			{
				...
				while(c.getTime().compareTo(dataFim)<=0)
				{
					String linhaSuja = home.obterDadosAquivoAsegurado(aseg, c.getTime());
	...
	{
		return new Label("Datos del Asegurado");
	}
}
