...
public class PersonasAseguradoraView extends Table {
	public PersonasAseguradoraView(Entidade entidade) throws Exception {
		super(5);
		...
		for (Iterator i = entidade.obterInferiores().iterator(); i.hasNext();)
		{
			...
			if (e instanceof Pessoa)
			{
				...
				Link link = new Link(e.obterNome(), new Action("visualizarDetalhesEntidade"));
		...
		novoLink.getAction().add("superiorId", entidade.obterId());
