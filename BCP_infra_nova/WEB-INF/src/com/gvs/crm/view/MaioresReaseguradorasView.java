...
public class MaioresReaseguradorasView extends PortalView
{
	...
	public MaioresReaseguradorasView(String tipoValor,Date dataInicio,Date dataFim, String situacao, double dolar, double euro, double real, double pesoArg, double pesoUru, double yen, Collection<String> dados, boolean porPais)
	{
	...
	public View getBody(User user, Locale locale, Properties properties) throws Exception
	{
		...
		table.addHeader("Por Pais");
		table.add(new Check("porPais", "true", porPais));
		
		Button button = new Button("Visualizar",new Action("maioresRaseguradoras"));
		table.addFooter(button);
		
		button = new Button("Generar Excel",new Action("maioresRaseguradoras"));
		button.getAction().add("excel", true);
		table.addFooter(button);
		
		button = new Button("Generar PDF",new Action("maioresRaseguradoras"));
		...
		if(dados.size() > 0)
		{
			...
			for(Iterator<String> i = dados.iterator() ; i.hasNext() ; )
			{
				...
				if(nome.toLowerCase().equals("null"))
				{
	...
	{
		return new Label("Mayores Reaseguradoras");
	}
