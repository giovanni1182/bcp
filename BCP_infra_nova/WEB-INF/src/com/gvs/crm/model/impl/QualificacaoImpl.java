...
public class QualificacaoImpl extends EntidadeImpl implements Qualificacao
{
	private int codigo;
	...
	public void incluir() throws Exception
	{
		...
		SQLUpdate insert = this.getModelManager().createSQLUpdate("crm","insert into qualificacao(id,codigo,descricao) values(?,?,?)");
	...
	public int obterCodigo() throws Exception 
	{
		if(this.codigo == 0)
		{
			SQLQuery query = this.getModelManager().createSQLQuery("crm","select descricao from qualificacao where id = ?");
	...
	{
		SQLUpdate update = this.getModelManager().createSQLUpdate("crm","update qualificacao set codigo = ? where id = ?");
	...
	{
		SQLUpdate update = this.getModelManager().createSQLUpdate("crm","update qualificacao set descricao = ? where id = ?");
