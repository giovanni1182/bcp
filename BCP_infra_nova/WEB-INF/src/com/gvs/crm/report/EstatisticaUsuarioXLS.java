...
public class EstatisticaUsuarioXLS extends Excel
{
	public EstatisticaUsuarioXLS(Date dataInicio, Date dataFim, Collection<Usuario> usuarios, String textoUsuario) throws Exception
	{
      ...
      celula.setCellValue("Estadística Utilización del Sistema");
      celula.setCellStyle(estiloTitulo);
      ...
      celula.setCellValue(new SimpleDateFormat("dd/MM/yyyy").format(dataInicio) + " hasta " + new SimpleDateFormat("dd/MM/yyyy").format(dataFim));
      celula.setCellStyle(estiloTitulo);
      ...
      for(Iterator<Usuario> i = usuarios.iterator() ; i.hasNext() ; )
      {
	      ...
	      {
	    	  String dataInicioStr = new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()) + " 00:00:00";
	    	  Date di = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").parse(dataInicioStr);
	    	  
	    	  String dataFimStr = new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()) + " 23:59:59";
	    	  Date df = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").parse(dataFimStr);
	    	  
	    	  String tempo = usuario.obterTempoConectado(di, df);
	    	  if(!tempo.equals(""))
	    	  {
		    	  celula = row.createCell(coluna);
		          celula.setCellValue(new SimpleDateFormat("dd/MM/yyyy").format(c.getTime()));
		          celula.setCellStyle(estiloTituloTabelaC);
      ...
      for(Iterator<Usuario> i = usuarios.iterator() ; i.hasNext() ; )
      {
		    ...
		    celula = row.createCell(3);
		    celula.setCellValue(usuario.obterChave());
		    celula.setCellStyle(estiloTextoE);
		    r = new Region(linha, (short)3, linha, (short)5);
		    planilha.addMergedRegion(r);
		    
		    celula = row.createCell(6);
		    celula.setCellValue(usuario.obterTempoConectado(dataInicio, dataFim));
